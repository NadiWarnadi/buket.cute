...
@section('title', 'Tambah Fuzzy Rule')
...
<div class="row mb-4">
    <div class="col-12">
        <div class="d-flex align-items-center gap-2">
            ...
            <h1 class="h3 mb-0">Tambah Fuzzy Rule</h1>
        </div>
    </div>
</div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">Form Tambah Rule</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.fuzzy-rules.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="intent" class="form-control @error('intent') is-invalid @enderror" 
                               placeholder="contoh: greeting, order_status, help" value="{{ old('intent') }}">
                        @error('intent')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted d-block mt-1">Nama unik untuk intent ini (contoh: greeting, order, help)</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="pattern" class="form-control @error('pattern') is-invalid @enderror" 
                                  rows="4" placeholder="keyword1|keyword2|/regex/i">{{ old('pattern') }}</textarea>
                        @error('pattern')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted d-block mt-1">
                            Pisahkan keyword dengan <code>|</code> atau gunakan regex: <code>/pattern/flags</code><br>
                            Contoh: <code>halo|hi|assalamualaikum|/^(hey|hello)/i</code>
                        </small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="confidence_threshold" step="0.01" min="0" max="1" 
                                   ...
                                   value="{{ old('confidence_threshold', 0.5) }}">
                            <span class="input-group-text">%</span>
                        </div>
                        @error('confidence_threshold')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted d-block mt-1">Nilai 0-1. Semakin tinggi, semakin ketat matching (default: 0.5)</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="response_template" class="form-control @error('response_template') is-invalid @enderror" 
                                  rows="4" placeholder="Contoh: Halo! Terima kasih telah menghubungi kami. Apa yang bisa kami bantu?">{{ old('response_template') }}</textarea>
                        @error('response_template')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted d-block mt-1">Pesan balasan otomatis (kosongkan jika hanya untuk kategorisasi)</small>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle"></i> Simpan Rule
                        </button>
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="testPattern" class="form-control form-control-sm" rows="3" placeholder="Masukkan pattern di sini">{{ old('pattern') }}</textarea>
                </div>
        ...
        <div class="card border-0 shadow-sm mt-3">
            <div class="card-header bg-light">
                <h5 class="mb-0 small fw-bold">Panduan Format Pattern</h5>
            </div>
            <div class="card-body small">
                <p><strong>Keyword Matching:</strong></p>
                <code>halo|hi|assalamualaikum</code>
                
                <p class="mt-3"><strong>Regex Pattern:</strong></p>
                <code>/^(hey|hello)/i</code>
                
                <p class="mt-3"><strong>Kombinasi:</strong></p>
                <code>halo|/^order/i|pesanan</code>

                <hr>
                <p class="mt-2 small"><strong>Flags Regex:</strong></p>
                <ul class="mb-0 small">
...
// Update test pattern saat pattern textarea berubah
