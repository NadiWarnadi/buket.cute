        ...
        <div class="chat-wrapper">
            ...
            <div class="chat-header">
                ...
                <div class="chat-header-actions">
                    ...
                    @if(!$isAdminHandled)
                        <button type="button" class="btn btn-sm btn-warning escalate-btn" 
                                id="escalateBtn"
                                data-customer-id="{{ $customer->id }}"
                                title="Escalate ke admin dengan alasan">
                            <i class="bi bi-exclamation-circle"></i> Escalate
                        </button>
                    @else
                        <button type="button" class="btn btn-sm btn-success resume-btn" 
                                id="resumeBtn"
                                data-customer-id="{{ $customer->id }}"
                                title="Resume ke AI handling">
                            <i class="bi bi-arrow-counterclockwise"></i> Resume AI
                        </button>
                    @endif
                </div>
            </div>
...
function toggleDelegation() {
    ...
    form.method = 'POST';
    form.action = `{{ route('admin.chat.toggle-delegation', ':id') }}`.replace(':id', customerId);
...
function escalateToAdmin() {
    ...
    if (!confirm('Escalate chat ke admin handling?')) return;

    const form = document.createElement('form');
    form.method = 'POST';
    form.action = `{{ route('admin.chat.escalate', ':id') }}`.replace(':id', customerId);
...
function resumeFromAdmin() {
    const customerId = {{ $customer->id }};
    
    if (!confirm('Yakin mengembalikan chat ke AI Bot?')) return;

    const form = document.createElement('form');
    form.method = 'POST';
    form.action = `{{ route('admin.chat.resume', ':id') }}`.replace(':id', customerId);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (escalateBtn) {
        escalateBtn.addEventListener('click', escalateToAdmin);
    }
    
    if (resumeBtn) {
        resumeBtn.addEventListener('click', resumeFromAdmin);
    }
});
</script>
@endsection